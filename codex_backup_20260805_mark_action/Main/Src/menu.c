...
#define TEST_DRIVE_RESULT_PAGE_COUNT 4U
...
static uint8_t test_drive_result_page = 0U;
typedef struct {
	uint32_t line_valid_records;
	uint32_t wide_records;
	uint32_t motor_records;
	uint32_t marker_counts[MARKER_EVENT_UNKNOWN + 1U];
	uint16_t sensor_min[SENSOR_COUNT];
	uint16_t sensor_max[SENSOR_COUNT];
	uint16_t left_speed_min;
	uint16_t left_speed_max;
	uint16_t right_speed_min;
	uint16_t right_speed_max;
	int16_t line_position_min;
	int16_t line_position_max;
	RunLogEvent_t last_event;
	uint8_t has_line_position;
	uint8_t has_motor_speed;
	uint8_t has_last_event;
} TestDriveRamSummary_t;
static TestDriveRamSummary_t test_drive_ram_summary;
...
static void Menu_DrawTestDriveReady(void)
{
	...
	Menu_DrawText(8U, 74U, 12U,
			LCD_COLOR_GRAY, LCD_COLOR_BLACK, "RAM ONLY - LOST ON RESET");
	Menu_DrawText(8U, 91U, 12U,
			LCD_COLOR_ORANGE, LCD_COLOR_BLACK,
			"L MAP   R SPEED   C 3S START");
	Menu_DrawText(8U, 106U, 12U,
			LCD_COLOR_RED, LCD_COLOR_BLACK, "NEXT RUN REPLACES RAM LOG");
	Menu_DrawText(8U, MENU_FOOTER_Y, 12U,
			LCD_COLOR_CYAN, LCD_COLOR_BLACK, "HOLD C TO GO BACK");
}
...
static void Menu_DrawTestDriveRunning(void)
{
	...
	Menu_DrawText(34U, 102U, 12U,
			...
			"RAM LOG ACTIVE - C STOPS");
}

static void Menu_BuildTestDriveRamSummary(void)
{
	const RunLogRecord_t *records = RunLog_GetRecords();
	const RunLogEvent_t *events = RunLog_GetEvents();
	uint32_t record_count = RunLog_GetRecordCount();
	uint32_t event_count = RunLog_GetEventCount();
	uint32_t record_index;
	uint32_t event_index;
	uint8_t sensor_index;

	memset(&test_drive_ram_summary, 0,
			sizeof(test_drive_ram_summary));
	for (sensor_index = 0U; sensor_index < SENSOR_COUNT; sensor_index++) {
		test_drive_ram_summary.sensor_min[sensor_index] = UINT16_MAX;
	}

	for (record_index = 0U; record_index < record_count; record_index++) {
		const RunLogRecord_t *record = &records[record_index];

		for (sensor_index = 0U; sensor_index < SENSOR_COUNT;
				sensor_index++) {
			uint16_t normalized = Sensor_NormalizeRaw(sensor_index,
					record->raw[sensor_index]);

			if (normalized
					< test_drive_ram_summary.sensor_min[sensor_index]) {
				test_drive_ram_summary.sensor_min[sensor_index]
						= normalized;
			}
			if (normalized
					> test_drive_ram_summary.sensor_max[sensor_index]) {
				test_drive_ram_summary.sensor_max[sensor_index]
						= normalized;
			}
		}

		if ((record->flags & RUN_LOG_FLAG_LINE_VALID) != 0U) {
			test_drive_ram_summary.line_valid_records++;
			if (test_drive_ram_summary.has_line_position == 0U) {
				test_drive_ram_summary.line_position_min
						= record->line_position;
				test_drive_ram_summary.line_position_max
						= record->line_position;
				test_drive_ram_summary.has_line_position = 1U;
			} else {
		...
		if ((record->flags & RUN_LOG_FLAG_WIDE_CENTER) != 0U) {
			test_drive_ram_summary.wide_records++;
		}
		if ((record->flags & RUN_LOG_FLAG_MOTOR_RUNNING) != 0U) {
			test_drive_ram_summary.motor_records++;
			if (test_drive_ram_summary.has_motor_speed == 0U) {
				test_drive_ram_summary.left_speed_min
						= record->left_steps_per_second;
				test_drive_ram_summary.left_speed_max
						= record->left_steps_per_second;
				test_drive_ram_summary.right_speed_min
						= record->right_steps_per_second;
				test_drive_ram_summary.right_speed_max
						= record->right_steps_per_second;
				test_drive_ram_summary.has_motor_speed = 1U;
			} else {
				if (record->left_steps_per_second
						< test_drive_ram_summary.left_speed_min) {
					test_drive_ram_summary.left_speed_min
							= record->left_steps_per_second;
				}
				...
				if (record->right_steps_per_second
						< test_drive_ram_summary.right_speed_min) {
					test_drive_ram_summary.right_speed_min
							= record->right_steps_per_second;
				}
	...
	if (record_count == 0U) {
		for (sensor_index = 0U; sensor_index < SENSOR_COUNT;
				sensor_index++) {
			test_drive_ram_summary.sensor_min[sensor_index] = 0U;
		}
	}
	for (event_index = 0U; event_index < event_count; event_index++) {
		uint8_t type = events[event_index].type;

		if (type <= (uint8_t)MARKER_EVENT_UNKNOWN) {
			test_drive_ram_summary.marker_counts[type]++;
		}
	}
	if (event_count != 0U) {
		test_drive_ram_summary.last_event = events[event_count - 1U];
		test_drive_ram_summary.has_last_event = 1U;
	}
}
...
{
	char text[40];
	uint32_t record_count = test_drive_last_status.record_count;
	uint32_t valid_percent = (record_count == 0U) ? 0U
			: ((test_drive_ram_summary.line_valid_records * 100U)
					+ (record_count / 2U)) / record_count;

	Menu_FillScreen(LCD_COLOR_BLACK);
	Menu_DrawText(8U, 2U, 16U,
			LCD_COLOR_CYAN, LCD_COLOR_BLACK, "8. TEST DRIVE");
	snprintf(text, sizeof(text), "RAM %u/%u",
			(unsigned int)(test_drive_result_page + 1U),
			(unsigned int)TEST_DRIVE_RESULT_PAGE_COUNT);
	Menu_DrawText(174U, 4U, 12U,
			LCD_COLOR_GREEN, LCD_COLOR_BLACK, text);
	ST7789_LCD_Driver.FillRect(&st7789_pObj, 0U, 21U,
			MENU_SCREEN_WIDTH, 1U, LCD_COLOR_CYAN);

	if (test_drive_result_page == 0U) {
		snprintf(text, sizeof(text), "STOP %s",
				TestDrive_GetStopReasonName(
						test_drive_last_status.stop_reason));
		Menu_DrawText(8U, 26U, 12U,
				LCD_COLOR_YELLOW, LCD_COLOR_BLACK, text);
		snprintf(text, sizeof(text), "TIME %lu.%01luS  REC %lu",
				(unsigned long)(test_drive_last_status.elapsed_ms / 1000U),
		...
		Menu_DrawText(8U, 60U, 12U,
				test_drive_last_status.dropped_frames == 0U
						? LCD_COLOR_GREEN : LCD_COLOR_RED,
				LCD_COLOR_BLACK, text);
		snprintf(text, sizeof(text), "FINAL POS %+d MASK %02X",
				(int)test_drive_last_status.line_position,
				(unsigned int)test_drive_last_status.sensor_state_mask);
		Menu_DrawText(8U, 77U, 12U,
				LCD_COLOR_CYAN, LCD_COLOR_BLACK, text);
		snprintf(text, sizeof(text), "FINAL L/R %u/%u SPS",
				(unsigned int)test_drive_last_status.left_steps_per_second,
				(unsigned int)test_drive_last_status.right_steps_per_second);
		Menu_DrawText(8U, 94U, 12U,
				LCD_COLOR_WHITE, LCD_COLOR_BLACK, text);
	} else if (test_drive_result_page == 1U) {
		snprintf(text, sizeof(text), "VALID %lu/%lu (%lu%%)",
				(unsigned long)test_drive_ram_summary.line_valid_records,
				(unsigned long)record_count,
				(unsigned long)valid_percent);
		Menu_DrawText(8U, 26U, 12U,
				valid_percent >= 90U ? LCD_COLOR_GREEN
						: LCD_COLOR_YELLOW,
				LCD_COLOR_BLACK, text);
		snprintf(text, sizeof(text), "LOST %lu  WIDE %lu",
				(unsigned long)(record_count
						- test_drive_ram_summary.line_valid_records),
				(unsigned long)test_drive_ram_summary.wide_records);
		Menu_DrawText(8U, 43U, 12U,
				LCD_COLOR_WHITE, LCD_COLOR_BLACK, text);
		if (test_drive_ram_summary.has_line_position != 0U) {
			snprintf(text, sizeof(text), "POS %d .. %d",
					(int)test_drive_ram_summary.line_position_min,
					(int)test_drive_ram_summary.line_position_max);
		} else {
			snprintf(text, sizeof(text), "POS NO VALID DATA");
		}
		Menu_DrawText(8U, 60U, 12U,
				LCD_COLOR_CYAN, LCD_COLOR_BLACK, text);
		snprintf(text, sizeof(text), "MOTOR %lu LOG FRAMES",
				(unsigned long)test_drive_ram_summary.motor_records);
		Menu_DrawText(8U, 77U, 12U,
				LCD_COLOR_WHITE, LCD_COLOR_BLACK, text);
		if (test_drive_ram_summary.has_motor_speed != 0U) {
			snprintf(text, sizeof(text), "SPS L%u-%u R%u-%u",
					(unsigned int)test_drive_ram_summary.left_speed_min,
					(unsigned int)test_drive_ram_summary.left_speed_max,
					(unsigned int)test_drive_ram_summary.right_speed_min,
					(unsigned int)test_drive_ram_summary.right_speed_max);
		} else {
			snprintf(text, sizeof(text), "SPS MOTOR NEVER STARTED");
		}
		Menu_DrawText(8U, 94U, 12U,
				LCD_COLOR_YELLOW, LCD_COLOR_BLACK, text);
	} else if (test_drive_result_page == 2U) {
		Menu_DrawText(8U, 26U, 12U,
				LCD_COLOR_GRAY, LCD_COLOR_BLACK,
				"NORM RANGE  BLACK0 WHITE1000");
		for (uint8_t index = 0U; index < 4U; index++) {
			snprintf(text, sizeof(text),
					"S%u %4u-%4u  S%u %4u-%4u",
					(unsigned int)index,
					(unsigned int)test_drive_ram_summary.sensor_min[index],
					(unsigned int)test_drive_ram_summary.sensor_max[index],
					(unsigned int)(index + 4U),
					(unsigned int)test_drive_ram_summary.sensor_min[index + 4U],
					(unsigned int)test_drive_ram_summary.sensor_max[index + 4U]);
			Menu_DrawText(8U, 43U + ((uint16_t)index * 17U), 12U,
					LCD_COLOR_WHITE, LCD_COLOR_BLACK, text);
		}
	} else {
		snprintf(text, sizeof(text), "S0 %lu  S7 %lu",
				(unsigned long)test_drive_ram_summary.marker_counts[
						MARKER_EVENT_EDGE_0],
				(unsigned long)test_drive_ram_summary.marker_counts[
						MARKER_EVENT_EDGE_7]);
		Menu_DrawText(8U, 26U, 12U,
				LCD_COLOR_WHITE, LCD_COLOR_BLACK, text);
		snprintf(text, sizeof(text), "BOTH %lu  CROSS %lu",
				(unsigned long)test_drive_ram_summary.marker_counts[
						MARKER_EVENT_BOTH],
				(unsigned long)test_drive_ram_summary.marker_counts[
						MARKER_EVENT_CROSS]);
		Menu_DrawText(8U, 43U, 12U,
				LCD_COLOR_CYAN, LCD_COLOR_BLACK, text);
		snprintf(text, sizeof(text), "UNKNOWN %lu  TOTAL %lu",
				(unsigned long)test_drive_ram_summary.marker_counts[
						MARKER_EVENT_UNKNOWN],
				(unsigned long)test_drive_last_status.event_count);
		Menu_DrawText(8U, 60U, 12U,
				LCD_COLOR_YELLOW, LCD_COLOR_BLACK, text);
		if (test_drive_ram_summary.has_last_event != 0U) {
			snprintf(text, sizeof(text), "LAST %s  DUR %luF",
					Menu_GetMarkerEventName((MarkerEventType_t)
							test_drive_ram_summary.last_event.type),
					(unsigned long)test_drive_ram_summary.last_event.duration_frames);
			Menu_DrawText(8U, 77U, 12U,
					LCD_COLOR_GREEN, LCD_COLOR_BLACK, text);
			snprintf(text, sizeof(text), "EDGE %02X ALL %02X WIDE %u",
					(unsigned int)test_drive_ram_summary.last_event.edge_union,
					(unsigned int)test_drive_ram_summary.last_event.full_union,
					(unsigned int)test_drive_ram_summary.last_event.max_wide_run);
		} else {
			snprintf(text, sizeof(text), "LAST NO MARK EVENT");
			Menu_DrawText(8U, 77U, 12U,
					LCD_COLOR_GRAY, LCD_COLOR_BLACK, text);
			snprintf(text, sizeof(text), "MARK COUNTS ARE RAM ONLY");
		}
		Menu_DrawText(8U, 94U, 12U,
				LCD_COLOR_WHITE, LCD_COLOR_BLACK, text);
	}
	Menu_DrawText(8U, MENU_FOOTER_Y, 12U,
			...
			"L/R PAGE  C READY/HOLD BACK");
}

static void Menu_FinishTestDriveResult(void)
{
	Motor_DriveStop();
	Sensor_Stop();
	TestDrive_GetStatus(&test_drive_last_status);
	Menu_BuildTestDriveRamSummary();
	test_drive_result_page = 0U;
...
static void Menu_ProcessTestDrive(UserInput_t input)
{
	if (test_drive_ui_state == TEST_DRIVE_UI_READY) {
		...
		if (Button_IsCenterPressed()) {
			/* The same physical press stops the run.  Consume its release so
			 * it cannot immediately dismiss the RAM result screen. */
		...
		test_drive_result_page = (test_drive_result_page == 0U)
				? (TEST_DRIVE_RESULT_PAGE_COUNT - 1U)
				: (test_drive_result_page - 1U);
		Menu_DrawTestDriveResult();
	} else if (input == INPUT_CMD_R_SINGLE) {
		test_drive_result_page = (test_drive_result_page + 1U)
				% TEST_DRIVE_RESULT_PAGE_COUNT;
		Menu_DrawTestDriveResult();
	}
}
